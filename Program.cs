...
try { 
    ...
    File.Move(foundStuff[i], foundStuff[i].Remove(foundStuff[i].Length - 7) + "1.prefab");
} } catch {
...
try { 
    ...
    File.Move(foundStuff1[i], foundStuff1[i].Remove(foundStuff1[i].Length - 8) + ".prefab");
} } catch {
    Console.WriteLine("unable to rename files, they're being used by a process already");
}}
